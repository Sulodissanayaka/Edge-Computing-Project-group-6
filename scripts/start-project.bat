...
set "FALLBACK_PYTHON=%PROJECT_ROOT%\.venv-runtime\Scripts\python.exe"
...
if exist "%PYTHON%" (
    "%PYTHON%" -c "import cv2, numpy, fastapi, uvicorn" >nul 2>&1
    if errorlevel 1 (
        echo [WARNING] The primary Python environment is incompatible.
        echo Using the bundled fallback environment instead.
        set "PYTHON=%FALLBACK_PYTHON%"
    )
)
...
if errorlevel 1 (
    echo.
    echo [ERROR] The available Python environment cannot load the backend packages.
    echo Recreate it by running:
    echo   python -m venv .venv
    echo   .venv\Scripts\python.exe -m pip install -r backend\requirements.txt
    echo.
    pause
